...
**Card Staircase** is a strategic card matching game implemented in Kotlin, 
designed for two players in a local hotseat setting, 
where players score points by matching their cards against a central staircase (see [Game Rules](#-game-rules)).
...
The project follows a clear separation between UI, game logic, and domain model,
...
- **Entity Layer (`entity` package)**  
  ...
  The `CardStaircase` class represents a single game instance and acts as the central state holder.
...
```
java -jar card-staircase-<version>.jar
```
...
```
io.github.zeyuyangdev.cardstaircase.MainKt
```
